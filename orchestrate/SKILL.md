...
Execute an already-accepted plan, milestone by milestone, to enterprise level. Each milestone: a **fresh implementer subagent** builds it, **one code-judo critic subagent** reviews it on both axes, Standards and Spec, reported as separate lists, you fix every finding, commit, update the plan. Close out with a whole-branch review and `plan-retire`.
